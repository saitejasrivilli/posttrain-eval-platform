...
| One reproducible load test with real measured numbers | Done | `scripts/load_test.py` (100 jobs / 10 concurrent submitters against the live stack). Real captured output in `benchmark/results/v0.8_load_test.json`: 100/100 succeeded, throughput **15.14 jobs/s**, queue-wait p50 **2.75s** / p95 **6.29s**, execution p50 **2.45ms** / p95 **6.8ms**, retry rate **0.0**. This is an **infrastructure load test** — the CPU toy-training/toy-evaluator execution path, not real GPU training. Real GPU training/checkpoint recovery is validated separately on a Tesla T4 (`V0.6_GPU_VALIDATION.md`); these numbers must not be conflated with each other |
...
| Defect found + fixed during hardening: checkpoint artifact dedup collision | Done | Building the repeatable smoke test surfaced a real latent bug: the toy trainer's checkpoint/final bytes were a pure function of hyperparameters, so two runs with identical config produced byte-identical content, correctly deduped by content-addressing (ADR 010/011) to a single `artifact_id` — but that collided with the `checkpoints_artifact_id` UNIQUE constraint, because a checkpoint record also needs distinct **execution-instance identity** (which run, which attempt), not just content identity. The original schema conflated those two identities, so every training job after the first FAILED. Content-addressing was not wrong — it correctly identified identical bytes; the checkpoint schema was missing a second identity dimension. Fixed by stamping run/attempt identity into saved artifacts (same nonce pattern the eval tests already use), so content identity and checkpoint-instance identity are now tracked separately. Covered by existing `tests/test_training_execution.py` + proven by two consecutive green smoke runs |
