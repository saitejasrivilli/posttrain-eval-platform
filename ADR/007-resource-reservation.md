...
**What rolls back the reservation transaction, precisely:** the re-verification check failing (job no longer eligible), OR any resource dimension's conditional capacity UPDATE affecting zero rows, OR the `reservations` INSERT itself failing (e.g. a duplicate `(job_id, attempt_number)` -- should be structurally impossible given the candidate-selection rule, but the PK constraint is a backstop, not the primary mechanism). Any of these rolls back the *entire* transaction -- capacity UPDATE(s) already applied within it are undone along with everything else, standard transactional semantics, no partial reservation (e.g. CPU reserved, GPU rejected) is ever left committed.

**What is explicitly NOT part of this transaction:** a Worker's later `claim()` call. Per ADR 009, scheduling and claiming are two separate atomic operations in two separate transactions, run by two different processes at two different times. This means a job claim *failing* (for whatever reason -- a race with another worker, the job having been cancelled in the interim) does **not** roll back or otherwise affect an already-committed reservation; the reservation simply remains `ACTIVE` and the job remains reserved-but-unclaimed, which is the same "stale reservation, case 1" situation ADR 009 already documents as an accepted operational limitation, not a new failure mode requiring reservation rollback logic.
