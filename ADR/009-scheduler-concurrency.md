...
**Explicit guarantee (required clarification A):** a Scheduler crash after a reservation successfully commits does **not** corrupt resource accounting, and does **not** require any recovery procedure to be re-claimed later -- the reservation is durable, `claim()` will find it whenever a Worker consumes the (already-published, per V0.2's at-least-once outbox) `job.queued` message, exactly as if the Scheduler that created it were still running. V0.4 makes no stronger claim than this. Specifically, V0.4 does **not** claim:
- that a crashed Scheduler's *in-flight, uncommitted* reservation attempt resumes automatically (it doesn't need to -- it rolled back, nothing to resume, the job simply gets re-ranked and re-considered by any Scheduler instance on the next pass, same as scenario 13 below);
- any automatic reconciliation process that scans for "reservations whose job never got claimed" and does something about them -- an unclaimed-but-reserved job (worker capacity shortfall, not a scheduler bug) is an accepted, documented limitation (see "stale reservations" above), not silently solved by implying recovery machinery that doesn't exist.
If a future version needs active reconciliation of abandoned reservations, that is new, explicitly-scoped work for that version -- not something V0.4 quietly relies on.
