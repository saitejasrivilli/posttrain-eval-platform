...
## 16. Resource conservation invariant (checked after every operation, not just at reservation time)
**Expected:** at every commit boundary, for every resource dimension: `0 <= allocated_* <= total_*`, and `allocated_* == SUM(amount) FROM reservations WHERE status='ACTIVE'`. This must hold after reservation, release, double-release-attempt, and Recovery-reclaim operations alike -- not "eventually," at every single commit.
**Why this is its own scenario, not implied by 1-15:** scenarios 1-15 each test one specific race or failure in isolation; this one is the cross-cutting property that would catch a bug scenarios 1-15 individually missed (e.g. an incremental-counter bug that happens to pass every individual race test but drifts after enough operations).
**Test:** a scripted sequence -- reserve for jobs A/B/C (partially consuming a 4-GPU pool), release B, attempt to release B again (must no-op), reserve for a new job D -- asserting the conservation invariant holds after *each* step, not just at the end. Concretely: 4 GPUs total; A=2, B=1, C=1 reserved (`allocated=4`); release B (`allocated=3`); double-release B (`allocated` still `3`, not `2`); confirm `allocated == SUM(active reservations)` at every one of these points.
