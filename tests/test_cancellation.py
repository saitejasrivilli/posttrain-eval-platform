...
def test_cancel_running_job_sets_flag_not_immediate_transition(db_session):
    ...
    repo.conditional_transition(
        db_session, job.id, valid_from=["QUEUED"], to_status=JobStatus.RUNNING.value
    )
...
def test_worker_honors_cancel_requested_at_checkpoint(db_session):
    ...
    observed by the worker's logic, not just stored inertly in the row."""
    job = service.create_job(db_session, JobCreate(job_type="sft"))
    ...
    # Worker is already past its own claim step (done above); process_job_message
    # would no-op here since status is RUNNING not QUEUED, so we exercise the
    # exact same checkpoint statements the worker uses post-claim.
    from app.repository import executions as executions_repo

    executions_repo.insert(db_session, job.id, "w1")
    ...
    repo.conditional_transition(db_session, job.id, valid_from=["RUNNING"], to_status=outcome)
    executions_repo.finalize(db_session, job.id, outcome)
